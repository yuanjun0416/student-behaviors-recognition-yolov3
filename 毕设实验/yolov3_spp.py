...
class YoloDetectionBlock(paddle.nn.Layer):
    ...
    # 使用多层卷积和BN提取特征
    def __init__(self,ch_in,ch_out,is_first=False,is_test=True):
    ...
    def _spp_module(self, input, is_test=True, name=""):
        output1 = input
        output2 = F.max_pool2d(
            x=output1,
            kernel_size=5,
            stride=1,
            padding=2)
        output3 = F.max_pool2d(
            x=output1,
            kernel_size=9,
            stride=1,
            padding=4)
        output4 = F.max_pool2d(
            x=output1,
            kernel_size=13,
            stride=1,
            padding=6)
        output = paddle.concat([output1, output2, output3, output4], axis=1)
        return output
    ...
    def forward(self, inputs):
        ...
        for j in range(2):
            if j == 0:
                ...
                out = self.conv3(out)
            if self.is_first and j == 1:
                out = self._spp_module(input=out,name='spp')
                out = self.conv2(out)
            out = self.conv1(out)
...
if __name__ == '__main__':
    ...
    detection = YoloDetectionBlock(ch_in=1024, ch_out=512,is_first='False')
    #print(detection)
